...
export type LibraryState = { state: 'no-library' | 'library' | 'playlists' | 'done', percent: number };
export const LibraryContext = createContext<{ state: LibraryState, requestUpdate: () => void }>({
    state: {
        state: "no-library",
        percent: -1
    },
