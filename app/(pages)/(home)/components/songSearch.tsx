import { SpotifyClientContext } from "@/app/(domain)/context";
import { Track } from "@spotify/web-api-ts-sdk";
import { useCallback, useContext, useRef, useState } from "react";
import Image from "next/image";
import Link from "next/dist/client/link";
import { searchSongs } from "../../demo/actions";
...
export default function SongSearch({ message = 'Search for a song...' }: { message?: string }) {
    ...
    return (
        <div className={"songSearch " + (results.length > 0 ? 'songSearchSuccess' : 'songSearch')}>
            <input type="text" placeholder={message} onChange={handleInputChange} onKeyDown={handleSubmit} />
