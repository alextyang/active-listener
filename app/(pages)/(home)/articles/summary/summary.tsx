import { ArtistHoverMenu, TrackHoverMenu, AlbumHoverMenu, ArticleHoverMenu } from "@/app/(components)/music/textualHoverMenu";
import { ArticleContext, TrackContext, TrackSyncContext } from "@/app/(domain)/app/context";
import { StreamableValue, readStreamableValue } from "ai/rsc";
import { useContext, useRef, useState, useEffect, ReactNode, ReactElement } from "react";
import { SummaryCard } from "../../../../(components)/app/summary/summary";
...
export function Summary() {
    ...
    useEffect(() => {
        ...
        if (!articles || articles.length === 0) {
            if (fetchState.state.state === 'idle' && trackContext?.track)
        ...
        const readStream = async (stream: StreamableValue<string, any>) => {
            ...
            fetchState.update({ state: 'idle', percent: -1 });
        };
    ...
    return (
        <SummaryCard src={trackContext?.track?.album.images[0].url}>
